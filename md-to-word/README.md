...
### 2026-09-08：正文段落默认两端对齐

**问题**：转换后的正文段落默认左对齐，用户需手动「全选 > 两端对齐」。

**修复**：在 `Normal` 样式上设置两端对齐（`JUSTIFY`），等价于 Word 的「全选 > 两端对齐」：

```python
style.paragraph_format.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
```

**说明**：这是**样式级**设置（写在 `styles.xml`，非逐段写），正文段落（无显式对齐）自动继承两端对齐；标题、图片、图注、分割线等显式居中的元素保持 `CENTER` 不受影响。
